...
5. **Corroborates** across brands: the same number posing as SBI, IndiGo and Paytm at once is one operation, drawn as a network.
...
![Across brands in 3D: the same number touching three brands, and who is advertising on the brand name](docs/screenshots/network.png)
...
```
  ...
  PASS  shared number linked across brands  · other_brands=['IndiGo', 'Paytm']
...
Every request is keyed by its parameters, so `make record` replaces the synthetic fixtures in place and the UI badge flips from "synthetic fixtures" to live. One sweep of one brand across five cities is 23 searches plus up to 10 reverse lookups, so the free plan's 250 searches a month covers about seven sweeps; `HELPLINE_MAX_CALLS` caps each sweep, and the health endpoint shows credits left. Recorded responses are replayed for free, so a re-sweep of a recorded brand costs nothing.
...
docs/adr/            four decisions · docs/demo-script.md the video's narration and shot list
```
...
- **Recorded from SerpApi on 24 Sep 2026: Zomato and HDFC Bank.** Every one of their fixtures is a real SerpApi response (labelled `recorded`, with its Search Archive link), so the demo video and the default console show Google as it was that day. What the live sweeps found:
  - **Zomato** (28 searches, 5 cities): 8 numbers flagged fake, 6 to check, 1 official-but-unlisted. Zomato publishes no phone helpline, yet five restaurant listings on zomato.com itself, named to rank for "customer care number", each hand out a personal mobile; an aggregator presents a Delhi landline as Zomato's "toll free" number; two Maps listings ("Zomato office", "BANTI ZOMATO SERVICE") carry mobiles and complaint pages. Every verdict is the tool's; an analyst confirms before anything is filed.
  - **HDFC Bank** (28 searches, 5 cities): 15 official (9 listed, 6 found on the bank's own domain, such as its missed-call numbers), 5 to check, 2 flagged. The real 1800 1600 / 1800 2600 stay green wherever bankbazaar or paisabazaar republish them; the flagged pair are an unclaimed "HDFC Bank" Maps listing in Sarojini Nagar with a personal mobile, and a landline that complaint sites discuss.
  - These two brands exposed six real-data bugs that the synthetic world could not (numbers such as "+91 1800 1601" and "1800 1600 / 1800 2600", North American numbers on aggregator pages, house numbers glued to a Maps phone, user-created pages on the brand's own domain, and reverse lookups that land on the brand's site). All are fixed with tests.
- **Synthetic for SBI, IndiGo and Paytm**, generated by `backend/scripts/make_synthetic_fixtures.py` in the exact response shapes. They are labelled `synthetic` in every file and in the UI, never carry an archive link, and their phone numbers are made up. They exist so the cross-brand graph, the lookalike and impersonating-ad cases, and the Paytm-in-Kolkata coverage gap are demonstrable offline; `make record` replaces them with real responses.
...
```bash
make docker     # builds the image and serves http://127.0.0.1:8787 in replay mode, no key needed
```

The same image deploys as-is: [Deploy to Render](https://render.com/deploy?repo=https://github.com/itssaharsh/helpline-watch) uses `render.yaml`; on Fly, `fly launch --copy-config --yes && fly deploy` uses `fly.toml`. Both start in replay mode, so a public instance answers every sweep from the recorded searches and spends no SerpApi credits. Set `SERPAPI_API_KEY` and `HELPLINE_MODE=auto` on the instance to go live.

## Verify it yourself

```bash
make test      # 50 unit + API tests (parsers, classifier, store, SSE stream, pack, patching)
...
- The classifier's precision is bounded by its signal set; that is why a `review` queue exists and why the pack is gated by a person. On the two recorded brands the known weak spot is the reverse lookup: keyword-stuffed spam blogs that contain the word "scam" next to any number count as complaint evidence, and a brand's second real number (HDFC's 1800 1601 on its own branch listings) is a one-digit lookalike until the analyst adds it to the list.
...
Public link goes here before submission. The video was recorded from the running app (replaying the recorded Zomato and HDFC Bank searches) and narrated by a local text-to-speech model; the narration and shot list are in `docs/demo-script.md`.
