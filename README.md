...
`demo/build/final.mp4` is produced by the storyboard in `demo/storyboard.json` (recorded from the running app on a virtual clock, narrated by a local text-to-speech model). Public link goes here before submission. The narration and shot list are in `docs/demo-script.md`.
