...
export function Network2D({ data, onNumber, height = 460 }: { data: NetworkData | null; onNumber: (n: string) => void; height?: number }) {
  ...
  if (!layout) return <Text size="2" className="muted">Sweep two or more brands to see numbers that pose as several of them at once.</Text>
  const shared = layout.nodes.filter((n) => n.type === 'number' && (n.brands ?? 0) >= 2).length
  return (
    <div>
      <Text as="p" size="2" className="muted" mb="3">Every suspicious number from the latest sweep of each brand. White nodes are brands, red are fakes, amber are numbers to check. Click a number to open it. <span style={{ color: '#F4F7FA' }}>{shared}</span> shared so far.</Text>
      <svg viewBox={`${layout.box.x} ${layout.box.y} ${layout.box.w} ${layout.box.h}`} preserveAspectRatio="xMidYMid meet" style={{ width: '100%', height, display: 'block' }} role="img" aria-label={`Scam network with ${shared} shared numbers`}>
        {layout.links.map((l, i) => { const hot = hover && (l.source.id === hover || l.target.id === hover); return <line key={i} className="edge" pathLength={1} x1={l.source.x} y1={l.source.y} x2={l.target.x} y2={l.target.y} stroke={hot ? '#F58A8A' : 'rgba(244,247,250,.22)'} strokeWidth={hot ? 2.5 : 1.5} style={{ animationDelay: `${i * 30}ms` }} /> })}
        {layout.nodes.map((n) => {
        ...
        {layout.nodes.filter((n) => n.type === 'brand' || (n.brands ?? 0) >= 2 || hover === n.id).map((n) => {
          ...
          return <text key={`l-${n.id}`} x={isBrand ? n.x : (n.x ?? 0) + r + 8} y={isBrand ? (n.y ?? 0) + r + 14 : (n.y ?? 0) + 4} textAnchor={isBrand ? 'middle' : 'start'} fontSize={isBrand ? 13 : 12} fontFamily="Host Grotesk, sans-serif" fontWeight={isBrand ? 600 : 500} fill={isBrand ? '#F4F7FA' : '#F58A8A'} stroke="#06090F" strokeWidth={4} style={{ paintOrder: 'stroke', pointerEvents: 'none' }}>{n.label}</text>
        })}
      </svg>
    </div>
  )
