...
export interface Sweep { id: string; brand_id: string; brand_name: string; started_at: string; finished_at: string | null; mode: string; city_ids: string[]; queries: string[]; calls_made: number; calls_planned: number; fixture_kinds: Record<string, number>; coverage: Coverage[]; findings: Finding[]; advertisers: Advertiser[]; errors: string[] }
