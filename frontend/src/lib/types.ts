...
export interface SerpItem { kind: 'ad' | 'answer' | 'local' | 'knowledge' | 'organic' | 'paa' | 'maps'; title: string | null; link: string | null; displayed_link: string | null; text: string | null; phone: string | null; rating: number | null; reviews: number | null; listing_type: string | null; address: string | null; place_id: string | null; unclaimed: boolean | null; question: string | null; numbers: { raw: string; norm: string }[] }
export interface SerpSnapshot { call_id: string; city_id: string | null; engine: string; query: string; hl: string; group: string; fixture_kind: string | null; archive_link: string | null; items: SerpItem[] }
export interface Coverage { city_id: string; planned: number; completed: number; failed: number }
export interface Sweep { id: string; brand_id: string; brand_name: string; started_at: string; finished_at: string | null; mode: string; city_ids: string[]; queries: string[]; calls_made: number; calls_planned: number; live_calls: number; cache_hits: number; fixture_kinds: Record<string, number>; coverage: Coverage[]; findings: Finding[]; advertisers: Advertiser[]; snapshots: SerpSnapshot[]; errors: string[] }
