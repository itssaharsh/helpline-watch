...
class SweepRunner:
    ...
    async def run(self, brand: Brand, cities: list[City], reverse_limit: int = DEFAULT_REVERSE_LIMIT) -> AsyncIterator[SweepEvent]:
        ...
        plan = build_plan(brand, cities, [], self.settings.max_calls)
        try:
        ...
        plan = build_plan(brand, cities, suggestions, self.settings.max_calls)
        ...
        for _ in plan.calls:
            ...
            if call.engine == "google":
                new_obs = surfaces.parse_google_search(res, call.city_id)
            elif call.engine == "google_maps":
                new_obs = surfaces.parse_google_maps(res, call.city_id)
            elif call.engine == "google_ads_transparency_center":
                advertisers = surfaces.parse_ads_transparency(res, brand)
            ...
            if advertisers and call.engine == "google_ads_transparency_center":
        ...
        for suspect in suspects:
            call = reverse_call(suspect.number_norm)
